...
| Git branch | Behavior |
| --- | --- |
| `develop` | Deploy to function `LATEST` only |
| `master` | Deploy to `LATEST`, then publish function version (description = commit ID), then update `production` alias to that version |
