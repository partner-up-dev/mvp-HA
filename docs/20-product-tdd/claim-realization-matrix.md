...
| --- | --- | --- | --- |
| 从一句话到可加入搭子请求 | natural-language and structured PR creation APIs, draft/publish lifecycle, persisted PR state | landing entrypoints, create page, create flows, publish orchestration | backend owns created object semantics; frontend owns entry UX |
...
| 搭子请求可传播、可回访 | canonical routes, detail-payload share metadata, share/meta endpoints, public config | route-scoped share orchestration, share UI, route navigation, history lists, page-level re-entry | backend owns canonical entity share metadata; frontend owns active share session and replay |
| 搭子请求需要可信度闭环 | confirmation rules, slot release, notifications, check-in, reliability persistence | CTA states, reminder subscription UI, check-in UI, explanation rendering | backend decides eligibility; frontend renders state and actions |
