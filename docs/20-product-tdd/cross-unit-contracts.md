...
- `/er/:eventId` is the Form Mode recommendation result route; it uses `l`, `d`, `t`, and repeated `p` query values as refresh-recoverable selection state and redirects back to `/e/:eventId` when required route state is missing or invalid
