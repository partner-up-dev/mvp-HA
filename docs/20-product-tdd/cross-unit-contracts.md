...
- `POST /api/pr/:id/waitlist` creates or reuses one `PENDING` partner slot for the current viewer when the PR is `FULL` and still before the join-lock boundary. It reuses the same identity, join-gate, Anchor Event participation frequency, and time-conflict guardrails as join, returns the refreshed public PR view plus auth payload, and does not make the viewer an active participant.
