...
- `GET /api/pr/:id/join-gates` is the PR join-gate projection contract. It returns the current viewer's join gates with per-gate resolved state. The projection reads PR-owned `joinGateConfig`; when that config is empty it returns a system fallback-confirm gate; booking-contact gate resolution comes from `pr_booking_contacts`; join-notice gate resolution comes from the current viewer's notice acceptance record.
- `POST /api/pr/:id/join-gates/:gateKey/resolve` resolves one concrete join gate before join. `JOIN_NOTICE` writes a viewer-scoped notice acceptance for the gate key and version. `BOOKING_CONTACT` validates and records the PR booking contact phone. The fallback-confirm gate is a frontend confirmation step and has no durable resolve command.
...
- The frontend PR join flow is owned by a reusable PR-domain flow component. PR detail, Form Mode matched handoff, and Form Mode no-match candidate actions provide their own button controls through slots while sharing join-gate resolution, join execution, auth payload application, and post-join recommendation prompts.
