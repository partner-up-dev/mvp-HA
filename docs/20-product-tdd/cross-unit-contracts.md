...
- `GET /api/events/:eventId/form-mode` is the Form Mode bootstrap contract; it returns the event snapshot, location gallery projection, available start options, published preset preference tags, and an optional backend-authored default selection from the nearest joinable PR under that event's time boundary.
