...
- PR 内留言的产品语义覆盖 Community PR 与 Anchor PR，但当前前端留言入口只在 Anchor PR 暴露。
...
- PR 内留言仅对当前活跃参与者可见；已退出或已释放的用户不应继续看到该 PR 留言内容。
...
- PR 内留言是非即时沟通层，不应引入在线状态、输入中、已读回执等聊天室语义。
- 通知订阅按剩余发送次数建模，而不是简单开关。
- 留言通知按“每个 PR / 每位收件者 / 每一波未处理留言最多 1 次”建模。
- 留言通知在真正发送前，必须再次验证收件者仍为该 PR 的当前活跃参与者。
