...
- 点击首页或页面底部“需要帮助”，进入 `/contact-support`；在该页可选择“联系客服”（报销/投诉）、跳转 `/contact-author` 反馈功能建议与 Bug，或通过底部“关于”入口前往 `/about`。
- 进入 `/about` 可查看产品名称、代码仓库地址，以及当前前端/后端 commit hash；并可通过“关注公众号”卡片弹窗查看公众号二维码。
