# TODOs of PartnerUp MVP-HA

- [ ] CICD 还有可以优化的（不需要检查层依赖、层清理、包清理）
- [ ] 有想要的功能？联系主创
- [ ] 强调无登录、无隐私收集
- [ ] Static Incremental Regeneration and Open graph
- [ ] 所有页面底部都可以联系主创、查看使用手册（使`用手册是首页的一部分）
- [ ] 没有想找搭子的人，点进来，也不要有太高的流失率
  - 比如说AI联想推荐、典型案例、避免误会（认为只有旅游才需要找所谓的搭子）
  - 讲清楚你的定位，避免过高的预期或者误会
  - 只有用户对你的产品有了比较清楚的认知，他们后面才可能会想起来你；而且这时候一定要让他们方便地找到你，比如做好SEO，比如推荐加入收藏夹（别人可能不记得你的名字，要选对标题）
- [ ] [NEED CONFIRM] 后端服务器冷启动会比较慢，Loading需要有两个阶段
- [ ] datetime picker 的编辑体验有待优化
- [ ] 跳转小红书App（最好能跳到笔记草稿页） <https://chatgpt.com/c/698b28a9-9990-83a8-bf9f-a00bd6158096>
- [ ] 分享到微信的“换一个”没有发现风格区别
- [ ] 小红书的海报生成风格还是太杂乱，提示词需要优化
  - 主要是文字重复
- [ ] 搭子请求NL解析肯定有问题，说周末找人，现在已经是周末结束了，竟然识别到2.9周一的时间窗口
- [x] 海报生成时还差几人应该以min计算而不是max
- [x] PRShare 也自动轮换，但是用户点击过一次切换按钮后就不了
- [ ] 首页也配置微信分享
- [ ] 在移动端的加载速度还有待优化
- [x] 可以处理企业微信自建应用的Webhook请求，返回解析结果（一般在 3-5 秒内，超时则返回已收到，稍后主动调用接口发送）
- [x] 每 72h 清理一次 OSS 上的 posters (使用 OSS 的生命周期规则即可)
- [x] remove LLMService
