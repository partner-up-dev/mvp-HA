...
- SharePR.vue: Carousel host for multiple share methods. Renders per-method components (ShareAsLink/ShareAsLink.vue, ShareToXiaohongshu/ShareToXiaohongshu.vue). Each method component internally owns its options, preview, and actions, ensuring proper state management and type safety. No duplicated types—imports ParsedPartnerRequest from @partner-up-dev/backend.
