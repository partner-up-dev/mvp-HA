<template>
  <PageScaffold class="anchor-pr-economy-page">
    <header class="page-header">
      <router-link
        v-if="id !== null"
        :to="anchorPRDetailPath(id)"
        class="back-link"
      >
    ...
    <template v-else-if="economy">
      ...
      <section
        v-if="economy.economyPreview.paymentModelApplied === 'A'"
      ...
      >
        ...
        <template v-else-if="reimbursement">
          ...
          <div v-if="economy.status === 'CLOSED'" class="reimburse-prompt-card">
...
const { data, isLoading, error } = useAnchorPREconomy(id);
const economy = computed(() => data.value);
...
const paymentModelText = computed(() => {
  if (economy.value?.economyPreview.paymentModelApplied === "A") {
    return t("prEconomy.model.modelA");
  }
  if (economy.value?.economyPreview.paymentModelApplied === "C") {
...
const discountRateText = computed(() =>
  formatDiscountRate(economy.value?.economyPreview.discountRateApplied),
);

const subsidyCapText = computed(() =>
  formatCurrency(economy.value?.economyPreview.subsidyCapApplied),
);

const policyScopeText = computed(() => {
  const scope = economy.value?.economyPreview.economicPolicyScopeApplied;
...
const bookingDeadlineText = computed(() =>
  formatDateTime(economy.value?.economyPreview.resourceBookingDeadlineAt),
);

const isDeadlinePassed = computed(() => {
  const deadline = economy.value?.economyPreview.resourceBookingDeadlineAt;
...
const bookingLockText = computed(() => {
  if (economy.value?.status === "LOCKED_TO_START") {
...
const cancellationPolicyText = computed(() =>
  economy.value?.economyPreview.cancellationPolicyApplied ||
  t("prEconomy.common.notSet"),
);
