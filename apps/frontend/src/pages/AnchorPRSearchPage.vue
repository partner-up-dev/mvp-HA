<template>
  <PageScaffold class="anchor-pr-search-page" data-page="anchor-pr-search">
    <PageHeader
      :title="pageTitle"
    ...
    >
      <template #top-actions>
        ...
        >
          {{ t("anchorPRSearch.actions.modifyCriteria") }}
        </Button>
      </template>
    </PageHeader>
    ...
    <div
      ...
      class="anchor-pr-search-page__state"
    >
      {{ t("common.loading") }}
    </div>
    ...
    <EmptyState
      ...
      :title="t('anchorPRSearch.loadEventsFailed')"
      :description="t('anchorPRSearch.loadEventsFailedHint')"
      icon="i-mdi-alert-circle-outline"
    >
      <template #actions>
        <Button tone="outline" @click="goEventPlaza">
          {{ t("anchorPRSearch.actions.goEventPlaza") }}
        </Button>
      </template>
    </EmptyState>
    ...
    <EmptyState
      ...
      :title="t('anchorPRSearch.emptyEventsTitle')"
      :description="t('anchorPRSearch.emptyEventsDescription')"
    ...
    <template v-else-if="!isResultMode">
      ...
      <div class="anchor-pr-search-page__footer-actions mt-4">
        ...
        <Button block :disabled="!canSubmitForm" @click="submitSearch">
          {{ t("anchorPRSearch.actions.search") }}
        </Button>
      </div>
    </template>

    <template v-else>
      <div v-if="isAutoRedirecting" class="anchor-pr-search-page__state">
        {{ t("anchorPRSearch.redirectingSingleResult") }}
      </div>
      ...
      <EmptyState
        ...
        :title="t('anchorPRSearch.loadFailed')"
      ...
      <template v-else>
        <p class="anchor-pr-search-page__summary">
          {{
            t("anchorPRSearch.resultSummary", { count: searchResults.length })
          }}
        </p>

        <div
          v-if="searchResults.length > 0"
          class="anchor-pr-search-page__result-list"
        >
          <AnchorPRSearchResultCard
        ...
        <EmptyState
          v-else
          :title="t('anchorPRSearch.emptyResultTitle')"
          :description="t('anchorPRSearch.emptyResultDescription')"
      ...
      <BottomDrawer
        ...
        :title="t('anchorPRSearch.drawerTitle')"
      ...
      >
        ...
        <template #footer>
          <div class="anchor-pr-search-page__drawer-actions">
            ...
            >
              {{ t("anchorPRSearch.actions.applyCriteria") }}
            </Button>
          </div>
        </template>
      </BottomDrawer>
...
const pageTitle = computed(() =>
  isResultMode.value
    ? t("anchorPRSearch.resultTitle")
    : t("anchorPRSearch.title"),
);
...
const pageSubtitle = computed(() => {
  ...
  if (!selectedEventTitle.value || !criteriaDateSummary.value) {
    return t("anchorPRSearch.resultSubtitleFallback");
  }
...
const searchErrorMessage = computed(
  () => searchQuery.error.value?.message ?? t("anchorPRSearch.loadFailedHint"),
);
...
.anchor-pr-search-page__state,
.anchor-pr-search-page__summary {
  margin: 0;
}
...
.anchor-pr-search-page__result-list {
  display: flex;
  flex-direction: column;
  gap: var(--sys-spacing-sm);
}

.anchor-pr-search-page__footer-actions,
.anchor-pr-search-page__drawer-actions {
  display: grid;
