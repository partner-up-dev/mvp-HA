...
- Prefer direct moves into the target structure over inventing new long-lived compatibility layers.
- Transitional facades are allowed only when they reduce risk materially.
- Do not add new modules to ambiguous legacy categories if a target home already exists.

## Retired Buckets

The following top-level buckets are retired and must not receive new product code:
...
Use explicit owners instead:

- app-wide and platform workflows -> `src/processes/*`
- cross-domain infrastructure -> `src/shared/*`
- domain-owned logic and UI -> `src/domains/<domain>/*`

If a module does not have a clear owner yet, stop and decide the owner first. Do not use a retired bucket as a temporary landing spot.

## Current Migration Status

Phase 6 is materially underway.

Completed:

- shared UI primitives now live under `src/shared/ui/*`
- PR slice migrated end-to-end under `src/domains/pr/*`
- event slice migrated under `src/domains/event/*`
- share slice migrated under `src/domains/share/*`
- admin slice migrated under `src/domains/admin/*`
- cross-domain config / poi / wechat reminder queries moved under `src/shared/*`
- landing surface sections moved under `src/domains/landing/*`
- app/platform workflows moved out of top-level `composables/*` into `src/processes/*` and `src/shared/*`
- top-level `features/*`, `widgets/*`, `entities/*`, `queries/*`, and `composables/*` have been reduced to retired/transitional shells

New work should follow the target structure by default, and should not reintroduce ownership into retired top-level buckets.
