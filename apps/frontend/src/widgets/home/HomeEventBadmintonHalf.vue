<template>
  <article class="event-half event-half--badminton">
    <div class="event-copy">
      <p class="event-kicker">
        {{ t("home.landing.eventCampaigns.badminton.kicker") }}
      </p>
      <h3>{{ t("home.landing.eventCampaigns.badminton.title") }}</h3>
      <p>{{ t("home.landing.eventCampaigns.badminton.description") }}</p>

      <RouterLink class="event-cta" :to="destination" @click="handleClick">
        {{
          t("home.landing.eventCampaigns.badminton.ctaWithLocation", {
    ...
    <div class="event-visual" aria-hidden="true">
      <div class="event-visual-frame">
        <span class="event-visual-icon i-mdi:badminton"></span>
        <span class="event-visual-label">
          {{ t("home.landing.eventCampaigns.badminton.imagePlaceholder") }}
        </span>
      </div>
    </div>
  </article>
...
const BADMINTON_INDEX = 0;

interface HomeEventBadmintonHalfProps {
  campaign: HomeEventCampaignViewModel;
}

const props = defineProps<HomeEventBadmintonHalfProps>();
const { t } = useI18n();
...
const locationText = computed(() => {
  ...
  return (
    ...
    t("home.landing.eventCampaigns.badminton.fallbackLocation")
  );
});
...
.event-half {
  --event-accent: var(--sys-color-primary);
  min-height: 0;
  display: grid;
