<template>
  <article class="event-half event-half--speaking">
    <div class="event-copy">
      <p class="event-kicker">
        {{ t("home.landing.eventCampaigns.speaking.kicker") }}
      </p>
      <h3>{{ t("home.landing.eventCampaigns.speaking.title") }}</h3>
      <p>{{ t("home.landing.eventCampaigns.speaking.description") }}</p>

      <RouterLink class="event-cta" :to="destination" @click="handleClick">
        {{ t("home.landing.eventCampaigns.speaking.cta") }}
        <span
    ...
    <div class="event-visual" aria-hidden="true">
      <div class="event-visual-frame">
        <span class="event-visual-icon i-mdi:account-voice"></span>
        <span class="event-visual-label">
          {{ t("home.landing.eventCampaigns.speaking.imagePlaceholder") }}
        </span>
      </div>
    </div>
  </article>
...
const SPEAKING_INDEX = 3;

interface HomeEventSpeakingHalfProps {
  campaign: HomeEventCampaignViewModel;
}

const props = defineProps<HomeEventSpeakingHalfProps>();
const { t } = useI18n();
...
.event-half {
  --event-accent: color-mix(
    in srgb,
    var(--sys-color-primary) 62%,
