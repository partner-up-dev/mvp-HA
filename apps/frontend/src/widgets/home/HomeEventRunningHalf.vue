<template>
  <article class="event-half event-half--running">
    <div class="event-copy">
      <p class="event-kicker">
        {{ t("home.landing.eventCampaigns.running.kicker") }}
      </p>
      <h3>
        {{
          t("home.landing.eventCampaigns.running.titleWithLocation", {
            location: titleLocation,
          })
        }}
      </h3>
      <p>{{ t("home.landing.eventCampaigns.running.description") }}</p>
    ...
    <div class="event-visual" aria-hidden="true">
      <div class="event-visual-frame">
        <span class="event-visual-icon i-mdi:run-fast"></span>
        <span class="event-visual-label">
          {{ t("home.landing.eventCampaigns.running.imagePlaceholder") }}
        </span>
      </div>
    </div>
  </article>
...
const RUNNING_INDEX = 1;

interface HomeEventRunningHalfProps {
  campaign: HomeEventCampaignViewModel;
}

const props = defineProps<HomeEventRunningHalfProps>();
const { t } = useI18n();
...
const titleLocation = computed(() => {
  ...
  return (
    ...
    t("home.landing.eventCampaigns.running.fallbackLocation")
  );
});

const ctaLabel = computed(() => {
  if (props.campaign.nextStartAtLabel) {
    return t("home.landing.eventCampaigns.running.ctaWithTime", {
      time: props.campaign.nextStartAtLabel,
    });
  }

  return t("home.landing.eventCampaigns.running.ctaFallback");
});
...
.event-half {
  --event-accent: var(--sys-color-secondary);
  min-height: 0;
  display: grid;
