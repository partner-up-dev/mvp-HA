...
- Admin 预订执行控制台: 新增 `/admin/booking-execution`，桌面端可查看满足预订执行条件（`READY/FULL/LOCKED_TO_START` + 已达到最小活跃人数 + 平台代订资源）的 Anchor PR 待处理列表、提交平台预订结果、释放无效预订联系人，并对执行审计与手动释放记录做客户端搜索。
